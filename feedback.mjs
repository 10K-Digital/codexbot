...
export function createFeedback({root,state,catalog,manage,enqueue,persist=()=>{},now=Date.now}){
 ...
 function publicState(){return {settings:data.settings,nextReviewAt:data.nextReviewAt,pendingCount:data.feedback.filter(f=>!f.processedAt).length,batch:data.batch?{jobId:data.batch.jobId,count:data.batch.items.length}:null,ratings:Object.fromEntries(data.feedback.map(f=>[f.messageId,{rating:f.rating,comment:f.comment}])),suggestions:data.suggestions.slice(-200),reviews:data.reviews.slice(-30)};}
 function rate(b){
 ...
 function startReview(){reconcile();if(data.batch)return {started:false,reason:'running'};const items=data.feedback.filter(f=>!f.processedAt).slice(0,40).map(f=>({...f,history:undefined}));if(!items.length)return {started:false,reason:'empty'};
 ...
 function review(job,b){const batch=data.batch;if(!batch||job?.id!==batch.jobId||job.feedbackReview!==batch.id)throw Error('Esta ferramenta exige uma tarefa de revisão de feedbacks.');
  if(b.action==='read')return {batchId:batch.id,feedback:batch.items,agents:catalog.agents.filter(a=>batch.items.some(f=>f.agentId===a.id)).map(a=>({id:a.id,name:a.name,instructions:a.description})),previousSuggestions:data.suggestions.filter(s=>batch.items.some(f=>f.agentId===s.agentId)).slice(-20).map(s=>({agentId:s.agentId,instruction:s.instruction,status:s.status,rationale:s.rationale}))};
 ...
 function tick(){reconcile();applyReady();if(!data.settings.enabled||now()<Date.parse(data.nextReviewAt))return;data.nextReviewAt=nextFeedbackReview(now());save();startReview();}
